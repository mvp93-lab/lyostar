  ...
  >
    ...
    >
      ...
      <button 
        @click="$emit('close')"
        :disabled="applying"
        class="absolute top-4 right-4 p-2 rounded-full text-slate-400 hover:text-white hover:bg-white/[0.06] transition-colors disabled:opacity-50"
        title="Close"
      >
        <X class="w-5 h-5" />
      </button>
      ...
      <!-- Shelves List -->
      <div class="flex-1 overflow-y-auto space-y-2 pr-1 min-h-[120px] scrollbar-thin">
      ...
      <!-- Quick Create New Shelf Accordion -->
      <div class="mt-4 pt-3 border-t border-white/[0.08]">
        <div v-if="!showNewShelfForm">
          <button
            ...
            :disabled="applying"
            class="w-full flex items-center justify-center gap-2 py-2 px-3 rounded-xl bg-white/[0.04] hover:bg-white/[0.08] border border-white/[0.08] text-xs font-medium text-slate-300 hover:text-white transition-all cursor-pointer disabled:opacity-50"
          >
      ...
      <div class="mt-4 pt-3 border-t border-white/[0.08] flex items-center justify-end gap-2.5">
        <button
          type="button"
          @click="$emit('close')"
          :disabled="applying"
          class="px-4 py-2 rounded-xl text-slate-400 hover:text-white hover:bg-white/[0.06] text-xs font-medium transition-colors cursor-pointer disabled:opacity-50"
        >
          Cancel
        </button>
        <button
          ...
          class="flex items-center gap-1.5 px-5 py-2 rounded-xl bg-glacier-500 hover:bg-glacier-400 text-slate-950 text-xs font-semibold shadow-md shadow-glacier-500/20 hover:shadow-glacier-500/30 transition-all cursor-pointer disabled:opacity-50"
        >
...
import { fetchShelves, fetchBookShelves, updateBookShelves, createShelf } from '../api/client'
...
const shelves = ref([])
const selectedShelfIDs = ref(new Set())
...
function isBookInShelf(shelfId) {
  return selectedShelfIDs.value.has(shelfId)
}
...
async function loadData() {
  ...
  try {
    ...
    selectedShelfIDs.value = new Set(bookShelfIds || [])
  } catch (err) {
...
async function handleApply() {
  ...
  try {
    const shelfIds = Array.from(selectedShelfIDs.value)
    await updateBookShelves(props.book.id, shelfIds)
    showToast(`Updated shelves for "${props.book.title}"`, 'success')
    emit('updated', { bookId: props.book.id, shelfIds })
    emit('close')
  } catch (err) {
    console.error('Failed to update book shelves:', err)
    error.value = err.message || 'Failed to update shelves'
  } finally {
...
async function handleCreateShelf() {
  ...
  try {
    ...
    selectedShelfIDs.value.add(created.id)
