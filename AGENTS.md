...
- Frontend: Vue 3 (Composition API, `<script setup>`), Vite, Tailwind CSS, Lucide Icons (`lucide-vue-next`).
  - Layout & Navigation: Calibre-Web inspired Sidebar Navigation Drawer (`Sidebar.vue`) organizing BROWSE (All Books, Continue Reading, Categories/Tags), SHELVES (custom user collections with book counts, + Create a Shelf, Manage Shelves), and MANAGEMENT (Upload, Users, Rescan, Sign out). Responsive off-canvas slide-out drawer on mobile/tablet and fixed sleek sidebar on desktop (`md:w-64`), with a decluttered, focused Topbar (`Navbar.vue`).
  ...
  - Web Routing & Deep-Linking: Vue Router 4 with HTML5 History mode (`createWebHistory()`) matching Calibre-Web URL schema (`/books`, `/continue-reading`, `/tags`, `/tags/:tag`, `/shelves/:id`, `/read/:id`, `/books/:id`, `/search?q=...`, `/users`, `/upload`, `/login`, `/setup`). Global navigation guards (`beforeEach`) enforce authentication, setup completion, and admin role restrictions. Full browser Back/Forward navigation support and independent deep-linking for in-browser reader and book details.
