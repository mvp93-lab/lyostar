...
- Custom Shelves & Collections: Per-user collections stored in SQLite `shelves` table (`id, name, description, user_id, is_public, created_at, updated_at`) and junction `shelf_books` table (`shelf_id, book_id, added_at`) with `ON DELETE CASCADE`. Supports private/public shelves, conflict detection for duplicate names per user (409 Conflict), batch book assignment via `ShelfSelectModal` (with local checkbox toggling, Cancel and Apply buttons, and instant toast/snackbar feedback), comprehensive management via `ShelvesManageModal`, and filtered library browsing via `GET /api/shelves/{id}/books`.
- Bookmarks & Highlights: Per-user bookmarks and notes stored in SQLite `bookmarks` (`id, user_id, book_id, location, title, progress, created_at`) and `highlights` (`id, user_id, book_id, location, selected_text, note, color, created_at, updated_at`) with `ON DELETE CASCADE`. Endpoints `/api/books/{id}/bookmarks` and `/api/books/{id}/highlights` guarded by `can_read` permission. Interactive Reader Side Drawer featuring Bookmarks, Notes (with 4 color presets: yellow, green, blue, pink), and Table of Contents (TOC) tabs with 1-click navigation.
...
- Frontend: Vue 3 (Composition API, `<script setup>`), Vite, Tailwind CSS, Lucide Icons (`lucide-vue-next`).
  - Layout & Navigation: Calibre-Web inspired Sidebar Navigation Drawer (`Sidebar.vue`) organizing BROWSE (All Books, Continue Reading, Categories/Tags, Authors), SHELVES (custom user collections with book counts, + Create a Shelf, Manage Shelves), and MANAGEMENT (Upload, Users, Rescan, Sign out). Responsive off-canvas slide-out drawer on mobile/tablet and fixed sleek sidebar on desktop (`md:w-64`), with a decluttered, focused Topbar (`Navbar.vue`).
